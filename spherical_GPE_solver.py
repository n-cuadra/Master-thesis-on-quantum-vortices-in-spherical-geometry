...
epsilon = 1e-5 #strength of bogoliubov excitations
...
def bog_exc(l, m, theta, phi, epsilon):
    ylm = pysh.expand.spharm_lm(l, m, theta, phi, normalization = 'ortho', kind = 'complex', degrees = False)
    yl_m = pysh.expand.spharm_lm(l, -m, theta, phi, normalization = 'ortho', kind = 'complex', degrees = False)
    return 1 + epsilon * (ylm + yl_m)
...
for i in range(N):
    for j in range(2*N):
        psi[i,j] = 10 * np.sqrt(gauss_density(theta[i], phi[j])) * np.exp(1.0j * phase(theta[i], phi[j])) #initalize psi with the upside down gaussian density
...
appable = plt.imshow(np.angle(psi), cmap = mycmap, vmin = -np.pi, vmax = np.pi)
