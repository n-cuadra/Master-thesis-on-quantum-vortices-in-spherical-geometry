...
omega_units = -0.207078 #rotating frequency in Hz
...
#the theta and phi values expanded over the whole grid as a N x 2N array
theta_grid = np.tile(theta[:, np.newaxis], reps = (1, 2 * N))
phi_grid = np.tile(phi[np.newaxis, :], reps = (N, 1))
